...
namespace SND.SMP.ItemTrackingReviews
{
    ...
    ) : AsyncCrudAppService<ItemTrackingReview, ItemTrackingReviewDto, int, PagedItemTrackingReviewResultRequestDto>(repository)
    {
        ...
        [HttpPost]
        [Route("api/PreRegisterItemV2/SA")]
        public async Task<OutPreRegisterItem> PreRegisterItemSPL(InPreRegisterItems input)
        {
            const string SUCCESS = "success";
            const string FAILED = "failed";
            string auto = "auto";
                    ...
                    if (ParcelGenerationUrl != null)
                    {
                        if (result.Errors.Count == 0)
                        {
                            ...
                            do
                            {
                                ...
                                {
                                    URL = ParcelGenerationUrl.Value.Trim(),
                                    RequestBody = JsonConvert.SerializeObject(splRequest),
                                    RequestDateTime = DateTime.Now
                                };

                                var content = new StringContent(JsonConvert.SerializeObject(splRequest), Encoding.UTF8, "application/json");
                                var saRequestMessage = new HttpRequestMessage
                                {
                                    Method = HttpMethod.Post,
                                    RequestUri = new Uri(ParcelGenerationUrl.Value.Trim()),
                                ...
                                if (httpstatus == HttpStatusCode.Created)
                                {
                                    ...
                                    if (saResult != null)
                                    {
                                        ...
                                        {
                                            newItemIdFromSPS = saResult.Airwaybill;
        ...
        public async Task<OutPreRegisterItem> PreRegisterItemsSPL(InPreRegisterItems input)
        {
            const string SUCCESS = "success";
            const string FAILED = "failed";
            string auto = "auto";
