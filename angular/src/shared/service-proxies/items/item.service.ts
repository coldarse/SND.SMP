...
import { PagedItemResultRequestDto, ItemDto } from './model';
...
export class ItemService {
    ...
    create(body: ItemDto){
        return this.http.post(
            this.url + '/api/services/app/Item/Create',
            body,
            this.options_
        ).pipe(
            retry(1),
    ...
    update(body: ItemDto){
        return this.http.put(
            this.url + '/api/services/app/Item/Update',
            body,
            this.options_
        ).pipe(
            retry(1),
    ...
    get(id: number){
        return this.http.get(
            this.url + `/api/services/app/Item/Get?Id=${id}`,
            this.options_
        ).pipe(
            retry(1),
            catchError(this.handleError),
