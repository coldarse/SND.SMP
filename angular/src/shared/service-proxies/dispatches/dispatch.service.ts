...
import { PagedDispatchResultRequestDto, DispatchDto } from './model';
...
export class DispatchService {
    ...
    create(body: DispatchDto){
        return this.http.post(
            this.url + '/api/services/app/Dispatch/Create',
            body,
            this.options_
        ).pipe(
            retry(1),
    ...
    update(body: DispatchDto){
        return this.http.put(
            this.url + '/api/services/app/Dispatch/Update',
            body,
            this.options_
        ).pipe(
            retry(1),
    ...
    get(id: number){
        return this.http.get(
            this.url + `/api/services/app/Dispatch/Get?Id=${id}`,
            this.options_
        ).pipe(
            retry(1),
            catchError(this.errorMessage.HandleErrorResponse),
