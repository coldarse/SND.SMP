...
export class PostalCountriesComponent extends PagedListingComponentBase<PostalCountryDto> {
  ...
  downloadTemplate() {
    let postalCountryHeaders: string[] = ["Postal Code", "Country Code"];

    let postalCountryTemplate: string[][] = [postalCountryHeaders, []];

    var wb = XLSX.utils.book_new();
    var ws = XLSX.utils.aoa_to_sheet(postalCountryTemplate);
    ws['!cols'] = this.fitToColumn(postalCountryHeaders);
    XLSX.utils.book_append_sheet(wb, ws, "Sheet 1");
    XLSX.writeFile(wb, `PostalCountriesUpload.xlsx`);
  }
